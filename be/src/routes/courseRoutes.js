...
const { upload } = require('../middleware/upload'); // Named export
...
router
  .route('/')
  .get(getCourses)
  .post(authorize('admin'), courseUpload, validate(createCourseRules), createCourse);

router
  .route('/:id')
  .get(getCourse)
  .put(authorize('admin'), courseUpload, validate(updateCourseRules), updateCourse)
